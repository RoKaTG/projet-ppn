...
MLP* create_mlp(int numLayers, int *layerSizes, double learningRate) {
    ...
    net->layerSizes = aligned_alloc(ALIGNMENT, numLayers * sizeof(int));

    // Allocating **ptrs for layers, weights, biases, etc.
    net->weights = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->biases = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->outputs = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->dOutputs = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->matprod = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->inputAdjoints = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->weightGradients = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    net->biasGradients = aligned_alloc(ALIGNMENT, (numLayers - 1) * sizeof(double *));
    ...
    // Parallelization for weight initialization
    #pragma omp parallel for    
    for (int i = 0; i < numLayers - 1; i++) {
        int rows = layerSizes[i + 1];
        int cols = layerSizes[i];
        net->weights[i] = aligned_alloc(ALIGNMENT, rows * cols * sizeof(double));
        net->biases[i] = aligned_alloc(ALIGNMENT, rows * sizeof(double));
        net->outputs[i] = aligned_alloc(ALIGNMENT, rows * sizeof(double));
        net->dOutputs[i] = aligned_alloc(ALIGNMENT, rows * sizeof(double));
        net->matprod[i] = aligned_alloc(ALIGNMENT, rows * sizeof(double));
        net->inputAdjoints[i] = aligned_alloc(ALIGNMENT, cols * sizeof(double));
        net->weightGradients[i] = aligned_alloc(ALIGNMENT, rows * cols * sizeof(double));
        net->biasGradients[i] = aligned_alloc(ALIGNMENT, rows * sizeof(double));
...
double squaredNorm(double *x, int n) {
    ...
    {
        __m256d sum_vec = _mm256_setzero_pd(); // Initialise un vecteur à zéro pour la somme dans chaque thread

        #pragma omp for nowait
        for (int i = 0; i <= n - 4; i += 4) {
            __m256d x_vec = _mm256_loadu_pd(&x[i]);  // Charge 4 éléments
            sum_vec = _mm256_add_pd(sum_vec, _mm256_mul_pd(x_vec, x_vec));  // Carré et somme
        }

        // Réduit la somme vectorisée en une seule valeur pour chaque thread
        double sum_array[4];
        _mm256_storeu_pd(sum_array, sum_vec);
        sum += sum_array[0] + sum_array[1] + sum_array[2] + sum_array[3];
    }

    // Gère les éléments restants si n n'est pas un multiple de 4
...
int feedforward(MLP *net, double *input, double *expected, int activation) {
    ...
    for (int i = 0; i < net->numLayers - 1; i++) {
        ...
        int N = 1;
        int K = net->layerSizes[i];

        cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, M, N, K, 1.0, 
                    net->weights[i], K, layerInput, N, 0.0, net->matprod[i], N);

        // Using vectorized ReLU and ReLU Prime for activation functions
        if (activation == 1) { // assuming 1 denotes ReLU
            if (i != net->numLayers - 2) { // Non-last layers
                relu_avx2(net->matprod[i], net->outputs[i], M);
                reluPrime_avx2(net->matprod[i], net->dOutputs[i], M);
...
void backpropagate(MLP *net, double *netInput, double lambda) {
    ...
    for (int i = lastLayerIndex; i >= 0; i--) {
        ...
        __m256d lrLambdaVec = _mm256_set1_pd(lambda);
        for (int j = 0; j < M * K; j++) {
            for (int k = 0; k < N; k += 4) {
                __m256d weightVec = _mm256_load_pd(&net->weights[i][j * N + k]);
                __m256d gradientVec = _mm256_load_pd(&net->weightGradients[i][j * N + k]);
                __m256d updateVec = _mm256_fmadd_pd(gradientVec, lrLambdaVec, weightVec); 
                _mm256_store_pd(&net->weights[i][j * N + k], updateVec);
            }
            // NOTE we apply L2 regularization with weight decay
            net->weights[i][j] -= lambda * net->weights[i][j];// * net->learningRate;
        }
